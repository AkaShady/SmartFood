 ...
 * @author will
 */
@WebServlet(urlPatterns = {"/CreateEmpresaServlet"})
public class CreateEmpresaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CreateEmpresaServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CreateEmpresaServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            empresasJpaController.create(empresas);
            response.sendRedirect("empresas/List.jsp"); 
        } catch (Exception ex) {
            Logger.getLogger(CreateEmpresaServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
