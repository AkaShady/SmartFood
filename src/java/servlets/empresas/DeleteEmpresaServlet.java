 ...
 * @author will
 */
@WebServlet(name = "DeleteEmpresaServlet", urlPatterns = {"/DeleteEmpresaServlet"})
public class DeleteEmpresaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DeleteEmpresaServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DeleteEmpresaServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String id = request.getParameter("id");
         ...
         try {
             empresasJpaController.destroy(Integer.parseInt(id));
             response.sendRedirect("empresas/List.jsp");
         } catch (RollbackFailureException ex) {
             Logger.getLogger(DeleteEmpresaServlet.class.getName()).log(Level.SEVERE, null, ex);
         } catch (Exception ex) {
             Logger.getLogger(DeleteEmpresaServlet.class.getName()).log(Level.SEVERE, null, ex);
         }
