...
import controller.pedido.exceptions.RollbackFailureException;
 ...
 * @author will
 */
@WebServlet(name = "UpdatePedidoServlet", urlPatterns = {"/UpdatePedidoServlet"})
public class UpdatePedidoServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UpdatePedidoServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UpdatePedidoServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        pedidos.setFuncionario(request.getParameter("funcionario"));
         ...
         try {
             pedidosJpaController.edit(pedidos);
             response.sendRedirect("pedidos/List.jsp");
         } catch (RollbackFailureException ex) {
             Logger.getLogger(UpdatePedidoServlet.class.getName()).log(Level.SEVERE, null, ex);
         } catch (Exception ex) {
             Logger.getLogger(UpdatePedidoServlet.class.getName()).log(Level.SEVERE, null, ex);
         }
