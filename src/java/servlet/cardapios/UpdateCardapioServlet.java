 ...
 * @author will
 */
@WebServlet(name = "UpdateCardapioServlet", urlPatterns = {"/UpdateCardapioServlet"})
public class UpdateCardapioServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UpdateCardapioServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UpdateCardapioServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        cardapios.setId(Integer.parseInt(id));
        ...
        CardapiosJpaController cardapiosJpaController = new CardapiosJpaController(utx, emf);
         ...
         try {
             cardapiosJpaController.edit(cardapios);
             response.sendRedirect("cardapios/List.jsp");
         } catch (RollbackFailureException ex) {
             Logger.getLogger(UpdateCardapioServlet.class.getName()).log(Level.SEVERE, null, ex);
         } catch (Exception ex) {
             Logger.getLogger(UpdateCardapioServlet.class.getName()).log(Level.SEVERE, null, ex);
         }
